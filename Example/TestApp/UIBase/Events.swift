...
typealias ControlEventCallback = (UIControl) -> Void

class ControlEvent: NSObject {
	var event: UIControl.Event
	var block: ControlEventCallback

	init(_ control: UIControl, _ event: UIControl.Event, _ block: @escaping ControlEventCallback) {
		self.event = event
		self.block = block
		super.init()
	...
	@objc
	func onEvent(_ control: UIControl) {
		self.block(control)
	}
